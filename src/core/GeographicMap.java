...
public class GeographicMap
{
	 ...
	 * it try offline geocoding first: if not possible, it will try online geocoding
	...
	protected String getReverseGeocoding(double lat, double lon) throws NotExistingCoordinatesException
	{
		try
		{
			return OfflineGeocoder.getReverseOfflineGeocoding(lat, lon);
		}
		catch(NotExistingCoordinatesException e)
		{
	...
	public GeographicMap(String osmFile, Locale area, AllowedVehicleTypes vehicle)
	{
		...
		//Set osm file which represents the osm map
		hopper.setDataReaderFile(this.osmFile);
